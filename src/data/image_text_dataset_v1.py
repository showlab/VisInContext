...
from .base import tuple_to_dict_w_text_image, get_dataset_size
...
class ImageTextDataset(BaseDataset):
    ...
    def init_wds_dataset(self, data_path):
        ...
        self.dataset = (
            ...
            .to_tuple("jpg;png;jpeg", "txt", "txt", handler=self.log_and_continue)
            .batched(self.batch_size)
            .map_tuple(self.preprocess_image_fn, self.preprocess_image_text_fn, self.preprocess_text_image_fn_iwt, handler=self.log_and_continue) #  handler=self.log_and_continue
            .map(tuple_to_dict_w_text_image)
        ).with_epoch(10000)
